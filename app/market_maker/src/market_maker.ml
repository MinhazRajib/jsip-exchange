...
(* One order we currently believe is resting on the book.
   ...
   [client_order_id] we chose ourselves, because the cancel RPC identifies
   an order by its *client* id, not its exchange id. [Order_accept] is the
   only event that shows us both ids together, so that is where we learn the
   pairing. *)
...
(* Our net position in [symbol] moves by [+size] when we buy and [-size]
   when we sell. [Side.sign] is exactly that (+1 for Buy, -1 for Sell), so
   the whole update is one multiply. *)
  ...
  Deferred.List.iter
    ...
    ~f:(fun level ->
      ...
      let order ~(side : Side.t) ~price_cents : Order.Request.t =
        ...
        ; size = Size.of_int config.size_per_level
        ; time_in_force = Day
        ; client_order_id = Client_order_id.Generator.next t.client_order_ids
        }
      in
      let%bind () =
        submit
